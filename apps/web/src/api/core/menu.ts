...
export namespace MenuApi {
  ...
  export interface MenuForm {
    ...
    /** 删除时间 */
    deleteAt?: Timestamp | null | undefined;
  }
...
export const getMenuTreeApi = async (param: any) => {
  return await requestClient.getWithParams('/menu/tree', param);
};
...
export const updateMenuApi = async (id: number, param: any) => {
  return await requestClient.put(`/menu/${id}`, param);
};
...
export const createMenuApi = async (param: any) => {
  return await requestClient.post('/menu', param);
};
...
export const deleteMenuApi = async (id: number) => {
  return await requestClient.delete(`/menu/${id}`);
};
...
// 添加清理函数
function cleanEmptyChildren(nodes: ApiTreeApi.ApiTreeNode[]) {
  for (const node of nodes) {
