...
const configTabs: TabOption[] = [
  ...
  {
    label: $t('page.monitor.scrapePool.config.tabs.ruleConfig'),
    value: 'rule-config',
  },
];
...
async function loadConfigData() {
  try {
    ...
    // 加载主配置数据
    const mainConfig = await getScrapePoolInstanceConfigApi({
    ...
    // 加载规则配置数据
    const ruleConfig = await getScrapePoolInstanceConfigApi({
      ip: data.value.host_ip,
    });
